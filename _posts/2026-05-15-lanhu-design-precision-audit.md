...
## 编辑前的 UI preflight gate

在任何修改前，先建立一张范围和责任表。它解决的是「看到了同一个视觉问题，就直接改一个 margin 或共享颜色」这类误判。

### 1. 页面范围矩阵

把需求中出现的每个页面或入口拆成独立行，不能用一张截图代表所有页面：

| 页面或入口 | 路由/组件 | 状态 | Tab | 主题 | viewport / density | 运行数据条件 | 范围状态 |
|---|---|---|---|---|---|---|---|
| 个人中心 | `{route / component}` | `{登录 / 未登录}` | `{Tab}` | `{日 / 夜}` | `{viewport / density}` | `{数据态}` | `in scope / unverified` |
| 首页 | `{route / component}` | `{状态}` | `{Tab}` | `{日 / 夜}` | `{viewport / density}` | `{数据态}` | `in scope / unverified` |

范围状态为 `unverified` 的行不能被省略。只有每个被点名的页面、状态和主题都完成检查，才能报告整项任务完成。

### 2. 视觉属性 owner 表

每个问题先确认真正负责它的代码层，再决定是否修改：

| 属性 | 需要追踪的 owner |
|---|---|
| inset / 位置 | window 或系统 inset、页面根、父容器 padding、carrier margin、运行时更新 |
| 可见大小 | carrier、bitmap canvas、alpha trim、density、`scaleType`、gravity |
| 背景 / 分割线 | 页面根、模块根、drawable、语义化日夜 Token、运行时分支 |
| 裁剪 / 显隐 | 父容器 clipping、子节点边界、绘制层级、`gone` 或状态绑定 |
| 内容 / 状态 | layout binding、adapter/model、图片 loader、placeholder、选中和日夜分支 |

只有最窄且能解释实际差异的 owner 才允许修改。两个页面显示了相同数值，不代表它们共享同一个实现 owner；跨页面共用修复前，必须先确认它们使用相同的 layout、组件、provider、Token 或运行时分支。

### 3. 稳定行为合同

在台账中记录需要保持不变的 data binding、点击区域、导航、状态切换和目标专属资源。视觉修复不能顺手替换这些行为或资产。

### 4. 评审冻结门

如果需求写了「先出图我评审」「不要动代码」或同义表达，本轮只允许生成视觉方案、比较表和修改建议，不得修改源码、资源、测试或生成产物。这个限制跨轮次持续有效，直到收到明确的实现批准；「继续」「快速处理」「看下」本身不等于批准。

最低评审产物应覆盖所有被点名的页面、页面范围矩阵、测量差值、owner 表和建议修改范围。
...
| 运行态证据 | 截图对应的路由、状态、主题、viewport/density、裁剪方式、时间、包或构建身份；来源不明的截图不能作为运行态证据 |

然后按顺序投喂 Prompt。不要一次要求 AI 同时修所有问题。先让 AI 固定设计身份，完成页面范围矩阵和 owner 表，再根据台账做窄范围补丁。
...
## Prompt 0.5：建立页面范围和 owner 表

这段 Prompt 放在几何台账之前，用来防止漏改页面、误改共享属性或把截图中的视觉结果归因到错误的代码层。

```text
任务：在任何代码修改前，建立 UI preflight gate。

输入：
- 用户点名的页面或入口：{页面列表}
- 当前问题：{位置 / 可见大小 / 背景 / 裁剪 / 状态 / 文字}
- 设计身份：{design id / version / updateTime}
- 代码入口：{route、Activity、Fragment、组件、布局、provider}
- 运行截图信息：{路由、状态、主题、viewport、density、裁剪方式、时间、包或构建身份}

执行：
1. 为每个页面 × 状态 × Tab × 主题 × viewport/density 建立范围矩阵，标记 in scope、out of scope 或 unverified。
2. 对每个视觉问题建立 owner 表，至少检查 inset、background、clip、Token 和 runtime binding。
3. 记录必须保持不变的 data binding、点击、导航、状态切换和目标专属资源。
4. 如果页面之间要共用一个修复，证明它们使用相同的 layout、组件、provider、Token 或运行时分支；否则分别处理。
5. 如果用户要求先评审再实现，只生成比较表、测量差值和建议修改范围，不修改任何代码、资源、测试或生成产物。
...
- 稳定行为合同。
- 截图来源和当前产物身份。
- 仍未确认的页面、状态、owner 或证据。
```

如果这一段发现某个页面、状态或截图来源仍未确认，后续结论必须降低到实际支持的证据等级，不能把未确认项默认为已经覆盖。
...
3. 建立页面范围矩阵，覆盖每个被点名的页面 × 状态 × Tab × 主题 × viewport/density，并标记 `in scope`、`out of scope` 或 `unverified`。
4. 建立 inset、background、clip、Token 和 runtime binding 的 owner 表；记录必须保持不变的 binding、点击、导航、状态切换和目标专属资源。
5. 记录 analyze 根宽高、设计到平台的换算和运行截图的 viewport/density/font scale/inset、裁剪方式与包或构建身份。
6. 建立页面盒模型和背景归属表，覆盖短内容、空列表、加载、隐藏和底部露色。
7. 建立同列模块横向边界台账；若有绝对定位、负向偏移、重叠或固定底部区，再建立绝对纵向坐标台账。
8. 建立模块间距台账，按视觉锚点相加所有 child、list、provider、item 和 decoration 间距。
9. 如果目标是列表页，建立完整列表契约；如果有固定高度卡片或有界文本行，建立文字槽位台账。
10. 建立颜色语义角色表和 Token 映射，检查日夜或皮肤变体。
11. 审查标题附件、卡片角标、分割线、运行态图片、圆角裁剪和图片边框绘制顺序。
12. 若存在重复组件，建立 peer contract 和 alpha-bounds 台账，检查每个密度桶。
13. 生成补丁目标清单；只有 `implement` 或 `commit` 模式才进入修改。若存在先评审后实现要求，停止在比较表和修改建议。
14. 修改后先审 diff，再构建、检查最终产物，并在真实路由复现每个被点名的状态。
15. 把设计与运行截图归一到同一坐标空间，输出最高证据等级和未满足的门槛。

输出格式：
- 审查范围。
- 页面范围矩阵、owner 表和稳定行为合同。
- 固定的设计身份、坐标空间和证据域完整性。
- 台账列表。
- 发现的问题，按风险排序。
- 补丁目标清单。
...
必须确认：
- 页面范围矩阵已经覆盖每个被点名的页面、状态、Tab、主题、viewport 和 density，没有把未验证项默认为已完成。
- inset、background、clip、Token 和 runtime binding 的 owner 已经确认；跨页面共用修复已经证明实现 owner 相同。
- data binding、点击、导航、状态切换和目标专属资源等稳定行为合同已经记录。
- 如果需求包含「先出图我评审」或「不要动代码」，实现冻结门已经遵守，且明确批准前没有修改源码、资源、测试或生成产物。
...
- 验证证据链已经关联到当前源码 revision、构建变体、产物路径或 hash、消费方解析结果和已安装包身份；历史构建不能替代当前产物证明。
- 已通过真实路由进入指定页面，覆盖指定 Tab、主题、数据/空/加载状态，并留存运行截图。
- 设计图和运行截图已归一到同一坐标空间，并记录稳定几何、文字槽位、颜色、资源和模块间距的差值。
- 已 exercise 行为合同中的点击、导航和绑定路径，并确认目标专属资源引用仍然存在。
...
最终报告不要把不同证据混成一句「已完成」。至少分成五行：

| 层级 | 应报告的事实 |
|---|---|
| 范围和行为 | 页面范围矩阵、owner 表、稳定 binding/点击/导航/状态合同，以及仍未验证的页面或状态 |
...
证据链应按以下顺序关联，而不是只列出几个孤立结果：

`源码或资源 diff → 聚焦测试/构建 → 打包资源或代码路径 → 消费方解析 → 最终 APK/AAB/HAP/Web bundle → 已安装包身份或 hash → 真实路由和状态复现 → 新截图比较`
...
蓝湖验收 Prompt 的核心，不是让 AI「看图更细」，而是让 AI 按固定证据格式工作，并且知道证据何时还不够。新增的 UI preflight gate 把页面范围、属性 owner、稳定行为和截图来源放到编辑之前，避免把跨页面问题误改成一个共享 margin，也避免用旧产物截图证明当前代码。
...
- 修改边界：先生成页面范围矩阵、owner 表和补丁目标清单，再改代码，改完先审 diff；共享属性用共享资源或样式表达，并用测试防止 peer 漂移。明确要求先评审时，保持实现冻结，直到收到明确批准。
- 验收等级：按「源码 diff → 构建测试 → 打包资源 → 消费方解析 → 安装包身份 → 真实路由和状态 → 新截图比较」逐段报告，缺一项就停在相应等级。
